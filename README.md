...
## Update 
This is the updated version of the original Repository [FloydHub](https://github.com/floydhub/dl-docker)
...
The Image is currently unavailable on the docker Hub. You need to manually Clone this 

#### Build the Docker image locally
You have to build the image locally. Note that this will take an hour or two depending on your machine since it compiles a few libraries from scratch.

```bash
git clone https://github.com/this-is-r-gaurav/ml-kit.git
cd ml-kit
```	

**CPU Version**
```bash
docker build -t mlkit:cpu -f Dockerfile.cpu .
```

This will build a Docker image named `mlkit` and tagged either `cpu`. Also note that the appropriate `Dockerfile.<architecture>` has to be used.


**CPU Version**
```bash
docker run -it -p 8888:8888 -p 6006:6006 -v /sharedfolder:/root/sharedfolder mlkit:cpu bash
```
...
Docker is supported on all the OSes mentioned here: [Install Docker Engine](https://docs.docker.com/engine/installation/) (i.e. different flavors of Linux, Windows and OS X). The CPU version (Dockerfile.cpu) will run on all the above operating systems. 
